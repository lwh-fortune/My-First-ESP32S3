...
bool irSendTriggered = false;        // 红外触发状态（云端下发用）
...
// ====================== MQTT 上传（已包含 Music_switch 和 Ir_launch） ======================
void uploadToOneNET() {
    ...
    jsonData += "\"Music_switch\":{\"value\":" + String(isPlayingMusic ? "true" : "false") + "},";
    jsonData += "\"Ir_launch\":{\"value\":" + String(irSendTriggered ? "true" : "false") + "}";  // 新增红外状态
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
    ...
    // 新增：处理云端红外发射指令
    if (params.containsKey("Ir_launch")) {
        bool val = params["Ir_launch"].as<bool>();
        Serial.printf("[DEBUG] 找到 Ir_launch = %d\n", val);
        if (val) {
            sendIR();                        // 执行红外发射
            irSendTriggered = false;         // 立即重置状态
            changed = true;
        }
    }
...
void sendIR() {
    ...
    Serial.println("红外信号已发送 (NEC 0x00FF02FD)");
}
...
void setup() {
    ...
    btnFan.attachLongPressStart(sendIR);      // 本地长按直接发射红外（不改变irSendTriggered）
